(function () {
  ...
  function settleTarget(targetPosition, scrollVelocity) {
    ...
    const slowScroll = Math.abs(scrollVelocity || 0) < 360;

    if (distance < 0.18 || slowScroll) return nearest;
  ...
  function buildWorkTimeline() {
    ...
    renderTick = () => {
      ...
      const detentForce = -detentDistance * detentEase * 0.043;

      wheelMotion.velocity += (targetForce + detentForce) * delta;
      wheelMotion.velocity *= Math.pow(0.8, delta);
    ...
    workTimeline = ScrollTrigger.create({
      ...
      trigger: section,
      start: "top top",
      end: () => `+=${Math.max(3800, window.innerHeight * 5.25)}`,
      pin: true,
      anticipatePin: 1,
      refreshPriority: 20,
      invalidateOnRefresh: true,
      onUpdate: (self) => {
        const carouselProgress = gsap.utils.clamp(0, 1, self.progress / 0.84);
        ...
        wheelMotion.targetOutro = gsap.utils.clamp(0, 1, (self.progress - 0.86) / 0.14);
      },
      onRefresh: (self) => {
        setupCardTrack();
        layoutWheel();
        const carouselProgress = gsap.utils.clamp(0, 1, (self.progress || 0) / 0.84);
        wheelMotion.targetSlot = Math.round(carouselProgress * (cards.length - 1));
        wheelMotion.targetPosition = wheelMotion.targetSlot;
        wheelMotion.currentPosition = wheelMotion.targetPosition;
        wheelMotion.targetOutro = gsap.utils.clamp(0, 1, ((self.progress || 0) - 0.86) / 0.14);
        wheelMotion.currentOutro = wheelMotion.targetOutro;

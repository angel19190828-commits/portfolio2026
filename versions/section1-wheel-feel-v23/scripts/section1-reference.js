(function () {
  ...
  function buildWorkTimeline() {
    ...
    renderTick = () => {
      ...
      const settleBlend = wheelMotion.targetOutro < 0.08
        ? gsap.utils.clamp(0, 1, (idleFor - 140) / 520)
        : 0;
      const effectiveTarget = gsap.utils.interpolate(wheelMotion.targetPosition, settleSlot, settleBlend * 0.86);
      const targetForce = (effectiveTarget - wheelMotion.currentPosition) * (0.052 + settleBlend * 0.01);
      ...
      const idleSettle = settleBlend * 0.024;
      const detentForce = -detentDistance * detentEase * (0.046 + idleSettle);

      wheelMotion.velocity += (targetForce + detentForce) * delta;
      wheelMotion.velocity *= Math.pow(gsap.utils.interpolate(0.795, 0.775, settleBlend), delta);

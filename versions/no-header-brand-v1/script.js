  ...
  if (!hasGsap) {
    ...
    if (hasOpening) {
      ...
      const getCornerRect = () => {
        ...
        probe.className = "hero-corner-asset";
      ...
      const finishOpening = () => {
        ...
        if (loadingAsset) {
          const target = getCornerRect();
          document.querySelector(".hero")?.appendChild(loadingAsset);
          loadingAsset.className = "hero-corner-asset";
      ...
      if (loadingAsset) {
        tl.fromTo(loadingAsset,
          ...
          { autoAlpha: 1, xPercent: -50, yPercent: -50, y: 0, scale: 1, rotation: 0, filter: "blur(0px) saturate(1)", duration: 0.68, ease: "power3.out" },
          0
        );
      }
      if (loadingAsset) {

(function () {
  ...
  function renderCards(progress) {
    const p = gsap.utils.clamp(0, 0.999, progress);
    const carouselProgress = gsap.utils.clamp(0, 0.999, p / 0.82);
    const position = carouselProgress * (cards.length - 1);

    cards.forEach((card, index) => {
      const offset = index - position;
      const distance = Math.abs(offset);
      const opacity = distance <= 0.18 ? 1 : gsap.utils.clamp(0, 1, 1 - (distance - 0.18) / 0.62);
      const y = offset * 34;
      const scale = 1 - Math.min(0.016, distance * 0.01);
      const blur = distance < 0.12 ? 0 : Math.min(1.8, distance * 1.6);
  ...
  function renderFrame(progress, rotation, immediate) {
    const p = gsap.utils.clamp(0, 0.999, progress);
    currentRotation = rotation;
    gsap.set(orbit, { rotation });
  ...
  function renderWheel(progress, immediate) {
    const p = gsap.utils.clamp(0, 0.999, progress);
    renderFrame(p, rotationFromProgress(p), immediate);
  }

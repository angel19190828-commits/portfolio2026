(function () {
  ...
  function applyOrbitDepth(position) {
    ...
    orbitItems.forEach((item, itemIndex) => {
      ...
      const color = gsap.utils.interpolate("rgba(46, 47, 49, .72)", "#b6a7ff", activeProximity);
  ...
  function renderCardStack(position) {
    ...
    cards.forEach((card, cardIndex) => {
      ...
      const settleY = selected ? -1.8 * landing + Math.sin(landing * Math.PI) * 0.45 : 0;
  ...
  function buildWorkTimeline() {
    ...
    renderTick = () => {
      ...
      const targetForce = (wheelMotion.targetPosition - wheelMotion.currentPosition) * 0.064;
    ...
    workTimeline = ScrollTrigger.create({
      ...
      trigger: section,
      start: "top top",
      end: () => `+=${Math.max(3600, window.innerHeight * 5.15)}`,
      pin: true,
      anticipatePin: 1,
      refreshPriority: 20,
      invalidateOnRefresh: true,
      onUpdate: (self) => {
        const carouselProgress = gsap.utils.clamp(0, 1, self.progress / 0.8);
      ...
      onRefresh: (self) => {
        setupCardTrack();
        layoutWheel();
        const carouselProgress = gsap.utils.clamp(0, 1, (self.progress || 0) / 0.8);

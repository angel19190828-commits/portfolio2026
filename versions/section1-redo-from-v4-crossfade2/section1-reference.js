(function () {
  ...
  function renderCards(progress) {
    ...
    cards.forEach((card, index) => {
      ...
      const opacity = distance <= 0.28 ? 1 : gsap.utils.clamp(0, 1, 1 - (distance - 0.28) / 0.52);
      const y = offset * 30;
      const blur = distance < 0.08 ? 0 : Math.min(2.5, distance * 2.2);
      const scale = 1 - Math.min(0.018, distance * 0.012);

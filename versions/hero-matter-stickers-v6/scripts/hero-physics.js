(function () {
  ...
  function applyPointerForce() {
    ...
    const radius = 178;
    const strength = Math.min(0.11, 0.012 + pointerSpeed * 0.0046);
    ...
    stickers.forEach((body) => {
      ...
      Body.setAngularVelocity(body, body.angularVelocity + (dx > 0 ? 1 : -1) * 0.026 * falloff);
    });
  }
  ...
  function start() {
    ...
    syncSize();
    makePointer();
    window.setTimeout(spawnStickers, 2200);

(function () {
  ...
  function renderCardStack(position) {
    ...
    const mobile = isMobile();
    const travel = mobile ? 112 : 132;

    cards.forEach((card, cardIndex) => {
      const offset = cardIndex - position;
      const absOffset = Math.abs(offset);
      const selected = cardIndex === activeCard;
      const y = offset * travel;

      const selectedAlpha = gsap.utils.clamp(0.42, 1, 1 - Math.max(0, absOffset - 0.08) / 0.68);
      const supportAlpha = absOffset < 0.38 ? gsap.utils.clamp(0, 0.38, (0.38 - absOffset) / 0.24) : 0;
      const autoAlpha = selected ? selectedAlpha : supportAlpha;

(function () {
  ...
  let renderTick = null;
  let settleCall = null;
  let trackpadBoostDelta = 0;
  let trackpadBoostFrame = null;
  let track = null;
  let cardSlot = 0;
  ...
  function renderCardStack(position) {
    ...
    cards.forEach((card, cardIndex) => {
      ...
      const scale = 1 - Math.min(absOffset, 1) * 0.007 + landing * 0.0025;
      const settleY = selected ? -1.35 * landing : 0;
  ...
  function buildWorkTimeline() {
    ...
    renderTick = () => {
      ...
      const detentForce = -detentDistance * detentEase * 0.046;

      wheelMotion.velocity += (targetForce + detentForce) * delta;
      wheelMotion.velocity *= Math.pow(0.795, delta);
      ...
      if (enteringDetent) {
        wheelMotion.detentPulse = gsap.utils.clamp(-0.026, 0.026, -wheelMotion.velocity * 0.16);
        wheelMotion.lastDetent = nearest;
      }
      wheelMotion.detentPulse *= Math.pow(0.78, delta);
    ...
    workTimeline = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        settleCall = gsap.delayedCall(0.18, () => {
          ...
          const max = cards.length - 1;
          const projected = gsap.utils.clamp(0, max, wheelMotion.currentPosition + wheelMotion.velocity * 1.25);
  ...
  window.addEventListener("wheel", (event) => {
    if (!workTimeline?.isActive || event.ctrlKey) return;
    const deltaY = event.deltaY || 0;
    const absY = Math.abs(deltaY);
    if (absY < 0.5 || absY > 56 || Math.abs(event.deltaX || 0) > absY * 0.9) return;

    const boost = absY < 18 ? 0.82 : 0.58;
    trackpadBoostDelta += deltaY * boost;
    trackpadBoostDelta = gsap.utils.clamp(-120, 120, trackpadBoostDelta);

    if (trackpadBoostFrame) return;
    trackpadBoostFrame = requestAnimationFrame(() => {
      trackpadBoostFrame = null;
      const nextY = gsap.utils.clamp(workTimeline.start, workTimeline.end, window.scrollY + trackpadBoostDelta);
      trackpadBoostDelta = 0;

(function () {
  ...
  function magneticPosition(rawPosition) {
    ...
    return clamped - distance * softened * 0.29;
  }
  ...
  function buildWorkTimeline() {
    ...
    wheelMotion.lastInputAt = performance.now();
    ...
    renderTick = () => {
      ...
      const idleFor = performance.now() - wheelMotion.lastInputAt;
      const predictedPosition = gsap.utils.clamp(0, maxPosition, wheelMotion.currentPosition + wheelMotion.velocity * 2.6);
      const settleSlot = gsap.utils.clamp(0, maxPosition, Math.round(predictedPosition));
      const settleBlendRaw = wheelMotion.targetOutro < 0.08
        ? gsap.utils.clamp(0, 1, (idleFor - 82) / 118)
        : 0;
      const settleBlend = settleBlendRaw * settleBlendRaw * (3 - 2 * settleBlendRaw);
      const effectiveTarget = gsap.utils.interpolate(wheelMotion.targetPosition, settleSlot, settleBlend * 0.9);
      const targetForce = (effectiveTarget - wheelMotion.currentPosition) * (0.052 + settleBlend * 0.052);
      ...
      const idleSettle = settleBlend * 0.012;
      const detentForce = -detentDistance * detentEase * (0.046 + idleSettle);

      wheelMotion.velocity += (targetForce + detentForce) * delta;
      wheelMotion.velocity *= Math.pow(gsap.utils.interpolate(0.795, 0.715, settleBlend), delta);
      ...
      if (enteringDetent) {
        wheelMotion.detentPulse = gsap.utils.clamp(-0.058, 0.058, -wheelMotion.velocity * 0.34);
        wheelMotion.lastDetent = nearest;
      }
      wheelMotion.detentPulse *= Math.pow(0.84, delta);
    ...
    workTimeline = ScrollTrigger.create({
      ...
      trigger: section,
      start: "top top",
      end: () => `+=${Math.max(4300, window.innerHeight * 6.25)}`,
      pin: true,
      anticipatePin: 1,
      refreshPriority: 20,
      ...
      onUpdate: (self) => {
        ...
        wheelMotion.lastInputAt = performance.now();
        wheelMotion.targetSlot = rawPosition;
        wheelMotion.targetPosition = rawPosition;

(function () {
  ...
  function magneticPosition(rawPosition) {
    ...
    return clamped - distance * softened * 0.26;
  }
  ...
  function buildWorkTimeline() {
    ...
    renderTick = () => {
      ...
      const positionEase = 1 - Math.pow(0.86, delta);
      const outroEase = 1 - Math.pow(0.82, delta);
      const previousPosition = wheelMotion.currentPosition;

      wheelMotion.currentPosition += (wheelMotion.targetPosition - wheelMotion.currentPosition) * positionEase;
      wheelMotion.currentOutro += (wheelMotion.targetOutro - wheelMotion.currentOutro) * outroEase;

      const velocity = wheelMotion.currentPosition - previousPosition;
      const nearest = Math.round(wheelMotion.currentPosition);
      const detentDistance = Math.abs(wheelMotion.currentPosition - nearest);
      const enteringDetent = nearest !== wheelMotion.lastDetent && detentDistance < 0.16 && Math.abs(velocity) > 0.006;
      if (enteringDetent) {
        wheelMotion.detentPulse = gsap.utils.clamp(-0.055, 0.055, -velocity * 0.18);
        wheelMotion.lastDetent = nearest;
      }
      wheelMotion.detentPulse *= Math.pow(0.76, delta);

      if (Math.abs(wheelMotion.targetPosition - wheelMotion.currentPosition) < 0.001) {
    ...
    workTimeline = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        wheelMotion.targetPosition = carouselProgress * (cards.length - 1);

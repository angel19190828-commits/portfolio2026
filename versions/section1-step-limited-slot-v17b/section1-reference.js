(function () {
  ...
  function slotTargetFromScroll(rawPosition, direction, currentSlot) {
    const max = cards.length - 1;
    const clamped = gsap.utils.clamp(0, max, rawPosition);
    ...
    let desiredSlot;
    if (direction < 0) {
      desiredSlot = gsap.utils.clamp(0, max, Math.floor(clamped + threshold));
    } else {
      desiredSlot = gsap.utils.clamp(0, max, Math.ceil(clamped - threshold));
    }

    const current = gsap.utils.clamp(0, max, Math.round(currentSlot));
    const step = gsap.utils.clamp(-1, 1, desiredSlot - current);
    const nextSlot = current + step;
    const boundedRaw = direction < 0
      ? gsap.utils.clamp(nextSlot, current, clamped)
      : gsap.utils.clamp(current, nextSlot, clamped);

    return gsap.utils.interpolate(boundedRaw, nextSlot, blend);
  }
  ...
  function buildWorkTimeline() {
    ...
    workTimeline = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        wheelMotion.targetSlot = slotTargetFromScroll(rawPosition, wheelMotion.scrollDirection, wheelMotion.targetSlot);

(function () {
  ...
  if (!hasGsap) {
    ...
    const hasOpeningAsset = !!document.querySelector(".opening__asset");
    ...
    if (hasOpening) {
      ...
      const finishOpening = () => {
        ...
        gsap.to(".hero-corner-asset", { autoAlpha: 0.88, filter: "blur(0px)", scale: 1, rotation: -2, duration: 0.18, overwrite: "auto" });
      ...
      gsap.set(".hero-corner-asset", { autoAlpha: 0.001, filter: "blur(0px)", scale: 1, rotation: -2 });
      if (hasOpeningAsset) {
        tl.fromTo(".opening__asset",
          { autoAlpha: 0, y: 18, scale: 0.94, rotation: -1.8, filter: "blur(22px) saturate(.9)" },
          { autoAlpha: 1, y: 0, scale: 1, rotation: 0, filter: "blur(0px) saturate(1)", duration: 0.68, ease: "power3.out" },
          0
        );
      }
      if (hasOpeningAsset && document.querySelector(".hero-corner-asset")) {
        tl.to(".opening__asset", {
          x: () => {
            const from = document.querySelector(".opening__asset").getBoundingClientRect();
            const to = document.querySelector(".hero-corner-asset").getBoundingClientRect();
          ...
          .to(".hero-corner-asset", { autoAlpha: 0.88, duration: 0.42, ease: "power2.out" }, 1.36)
          .to(".opening__asset", { autoAlpha: 0, duration: 0.42, ease: "power2.out" }, 1.36);
      }
      tl.to(".opening", { autoAlpha: 0, filter: "blur(8px)", duration: 0.44, ease: "power3.inOut" }, 1.7)

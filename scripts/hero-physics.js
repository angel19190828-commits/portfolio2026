(function () {
  ...
  const stickerSources = [
    ...
    { src: "./assets/stickers/web/notebook.png", width: 190, ratio: 0.75, restitution: 0.26 },
    { src: "./assets/stickers/web/tiny-heart.png", width: 92, ratio: 0.83, restitution: 0.38 },
  ];
  const angelInitialSticker = { src: "./assets/angel-yu.png", width: 360, ratio: 0.26, restitution: 0.2 };
  const initialWebStickerSources = stickerSources.slice(0, 4);
  ...
  function makePointer() {
    pointerBody = Bodies.circle(-999, -999, 68, {
      isStatic: true,
  ...
  function randomStickerConfig(pool) {
  ...
  function spawnInitialStickers() {
    ...
    configs.forEach((config, index) => {
      ...
      createSticker(config, index, {
        x: width * (0.18 + column * 0.64) + (Math.random() - 0.5) * 42,
        y: -80 - index * 24 - Math.random() * 92,
      });
    });
  }
  ...
  function start() {
    ...
    syncSize();
    makePointer();
    const openingCookieSeen = document.cookie.split(";").some((item) => item.trim() === "angelPortfolioOpeningSeen=true");
    let openingStorageSeen = false;
    try {
      openingStorageSeen = window.localStorage.getItem("angel-portfolio-opening-seen") === "true";
    } catch (error) {}
    const shouldWaitForOpeningTail = !!document.querySelector(".opening") && !openingCookieSeen && !openingStorageSeen;
    if (shouldWaitForOpeningTail) {
      window.addEventListener("angel:opening-tail", spawnInitialStickers, { once: true });
      window.setTimeout(spawnInitialStickers, 2300);
    } else {
      window.setTimeout(spawnInitialStickers, 100);
    }
